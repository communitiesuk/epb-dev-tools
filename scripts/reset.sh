...
if [[ -z $(confirm "Do you want to reset and configure the dev environment?") ]]; then
  ...
  until_accepting_connections "epb-dev-tools_epb-auth-server-db-1"
  echo "Auth DB OK"
  until_accepting_connections "epb-dev-tools_epb-feature-flag-db-1"
  echo "Toggles DB OK"
  until_accepting_connections "epb-dev-tools_epb-register-api-db-1"
  echo "API DB OK"
  until_accepting_connections "epb-dev-tools_epb-data-warehouse-db-1"
